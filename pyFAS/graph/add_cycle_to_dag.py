...
"""
This code is copied from the following github repository: https://github.com/zhenv5/breaking_cycles_in_noisy_hierarchies

All credits go to the original contributors of that repository. 

Copy of the BSD Licence

                                                BSD Licence.

This work is supported by the National Science Foundation of United States under grant CCF-1645599 and IIS-1550302.
...
def introduce_cycles_2_DAG(graph_file, num_extra_edges, path_length):
    ...
    extra_edges_file = graph_file[:len(graph_file) - 6] + "_extra_" + str(num_extra_edges) + "_path_len_" + str(
        path_length) + ".edges"
    graph_with_extra_edges_file = graph_file[:len(graph_file) - 6] + "_graph_w_extra_" + str(
    ...
    from graph.file_io import write_pairs_to_file
...
if __name__ == "__main__":
    ...
    parser.add_argument("-g", "--graph_file", default="/home/karan-m/Documents/Research/variant_1/Adam-Can-Play-Any-Strategy/pyFAS/files/gnm_300_2500.edges", help="input graph file name (edges list) DAG")
    parser.add_argument("-k", "--number", default=1, type=int, help="# extra edges added to the DAG")
    parser.add_argument("-l", "--length", default=0, type=int,
                        help="thresold d to control path length (<=0: no constraints on path length, otherwise less than this threshold)")
